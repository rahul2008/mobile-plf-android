...
import com.philips.platform.baseapp.screens.dataservices.database.datatypes.ConsentDetailStatusType;
...
public class OrmDeleting {
    ...
    private void insertDefaultConsentAndSyncBit() {
        try {
            consentDetailDao.createOrUpdate(new OrmConsentDetail(ConsentDetailType.SLEEP, ConsentDetailStatusType.REFUSED,ConsentDetail.DEFAULT_DOCUMENT_VERSION,
                    ConsentDetail.DEFAULT_DEVICE_IDENTIFICATION_NUMBER));
            consentDetailDao.createOrUpdate(new OrmConsentDetail(ConsentDetailType.TEMPERATURE, ConsentDetailStatusType.REFUSED,ConsentDetail.DEFAULT_DOCUMENT_VERSION,
                    ConsentDetail.DEFAULT_DEVICE_IDENTIFICATION_NUMBER));
            consentDetailDao.createOrUpdate(new OrmConsentDetail(ConsentDetailType.WEIGHT, ConsentDetailStatusType.REFUSED,ConsentDetail.DEFAULT_DOCUMENT_VERSION,
                    ConsentDetail.DEFAULT_DEVICE_IDENTIFICATION_NUMBER));
            syncDao.createOrUpdate(new OrmDCSync(OrmTableType.CONSENT.getId(), OrmTableType.CONSENT.getDescription(), true));
    ...
    private void insertDefaultSettingsAndSyncBit() {
        try {
            ...
            syncDao.createOrUpdate(new OrmDCSync(OrmTableType.SETTINGS.getId(), OrmTableType.SETTINGS.getDescription(), true));
        } catch (SQLException e) {
            e.printStackTrace();
        }
    }
    ...
    private void insertDefaultUCSyncBit() {
        try {
            syncDao.createOrUpdate(new OrmDCSync(OrmTableType.CHARACTERISTICS.getId(), OrmTableType.CHARACTERISTICS.getDescription(), true));
        } catch (SQLException e) {
            e.printStackTrace();
        }
    }
