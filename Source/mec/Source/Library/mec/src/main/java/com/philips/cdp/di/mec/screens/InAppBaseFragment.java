...
import com.philips.cdp.di.mec.cart.MECCartListener;
...
public abstract class InAppBaseFragment extends Fragment implements BackEventListener {
    ...
    protected MECListener mMecListener;
    ...
    /*protected MECCartListener mProductCountListener = new MECCartListener() {
        @Override
    ...
    public void setActionBarListener(ActionBarListener actionBarListener, MECListener mecListener) {
        mActionbarUpdateListener = actionBarListener;
        mMecListener = mecListener;
    }
    ...
    public void addFragment(InAppBaseFragment newFragment,
        ...
        if (mActionbarUpdateListener == null || mMecListener == null)
            new RuntimeException("ActionBarListner and IAPListner cant be null");
        else {

            newFragment.setActionBarListener(mActionbarUpdateListener, mMecListener);
            if (getActivity() != null && !getActivity().isFinishing()) {
                ...
                MECLog.d(MECLog.LOG, "Add fragment " + newFragment.getClass().getSimpleName() + "   ("
                        + newFragmentTag + ")");
            }
    ...
    public void showProductCatalogFragment(String fragmentTag) {
        Fragment fragment = getFragmentManager().findFragmentByTag(MECProductCatalogFragment.TAG);
        if (fragment == null) {
            getFragmentManager().popBackStack(fragmentTag, FragmentManager.POP_BACK_STACK_INCLUSIVE);
            addFragment(MECProductCatalogFragment.createInstance(new Bundle(),
                    AnimationType.NONE), MECProductCatalogFragment.TAG,true);
        } else {
            getFragmentManager().popBackStack(MECProductCatalogFragment.TAG, 0);
        }
    }

    public void clearFragmentStack() {
        if (getActivity() != null && getActivity() instanceof MECActivity && !getActivity().isFinishing()) {
    ...
    protected boolean isNetworkConnected() {
        ...
        if (mContext != null && !NetworkUtility.getInstance().isNetworkAvailable(connectivityManager)) {
            NetworkUtility.getInstance().showErrorDialog(mContext,
                    getFragmentManager(), mContext.getString(R.string.mec_ok),
                    mContext.getString(R.string.mec_you_are_offline), mContext.getString(R.string.mec_no_internet));
            return false;
        } else {
            return true;
        }
    ...
    public void updateCount(final int count) {
        if (mMecListener != null) {
            mMecListener.onGetCartCount(count);
            hideProgressBar();
        }
    }
    ...
    public void setCartIconVisibility(final boolean shouldShow) {
        if (mMecListener != null) {
            if(isUserLoggedIn()) {
                mMecListener.updateCartIconVisibility(shouldShow);
            }else{
                mMecListener.updateCartIconVisibility(false);
            }
        }
    }
...
/*
    ...
    public boolean shouldGiveCallBack(){
       return MECUtility.getInstance().getMecOrderFlowCompletion()!=null && !MECUtility.getInstance().getMecOrderFlowCompletion().shouldPopToProductList();
    }

    public void sendCallback(boolean isSuccess){
        moveToVerticalAppByClearingStack();
        if(isSuccess){
            MECUtility.getInstance().getMecOrderFlowCompletion().didPlaceOrder();
        }else{
            MECUtility.getInstance().getMecOrderFlowCompletion().didCancelOrder();
        }
    }

    protected boolean isUserLoggedIn(){
        return  MECUtility.getInstance().getUserDataInterface() != null && MECUtility.getInstance().getUserDataInterface().getUserLoggedInState() == UserLoggedInState.USER_LOGGED_IN ;
    }
}
