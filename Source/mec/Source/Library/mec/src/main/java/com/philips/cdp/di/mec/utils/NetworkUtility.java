...
import com.philips.cdp.di.mec.screens.ErrorDialogFragment;
...
public class NetworkUtility {
    ...
    public void showErrorDialog(Context context, FragmentManager pFragmentManager,
        ...
        MECAnalytics.trackAction(MECAnalyticsConstant.SEND_DATA,
                MECAnalyticsConstant.IN_APP_NOTIFICATION_POP_UP, pErrorDescription);
    ...
    public void showErrorMessage(final Message msg, FragmentManager pFragmentManager, Context context) {
        ...
        if (msg.obj instanceof MECNetworkError) {
            MECNetworkError error = (MECNetworkError) msg.obj;
            showErrorDialog(context, pFragmentManager, context.getString(R.string.mec_ok),
    ...
    public void showDialogMessage(String title, String description, FragmentManager pFragmentManager, Context context, final AlertListener alertListener) {
        if (context == null) return;
        showErrorDialogPayment(context, pFragmentManager, context.getString(R.string.mec_ok),
                title,
    ...
    public String getErrorTitleMessageFromErrorCode(final Context context, int errorCode) {
        String errorMessage;
        if (errorCode == MECConstant.MEC_ERROR_NO_CONNECTION) {
            errorMessage = context.getString(R.string.mec_you_are_offline);
        } else {
            errorMessage = context.getString(R.string.mec_server_error);
        }
        return errorMessage;
    }

    public String getErrorDescriptionMessageFromErrorCode(final Context context,
                                                          MECNetworkError error) {
        if (error.getIAPErrorCode() != MECConstant.MEC_ERROR_NO_CONNECTION
                && !TextUtils.isEmpty(error.getMessage())) {
            MECAnalytics.trackAction(MECAnalyticsConstant.SEND_DATA,
                    MECAnalyticsConstant.ERROR, MECAnalyticsConstant.SERVER + error.getIAPErrorCode() + "_" + error.getMessage());
        ...
        if (errorCode == MECConstant.MEC_ERROR_NO_CONNECTION) {
            errorMessage = context.getString(R.string.mec_no_internet);
        } else if (errorCode == MECConstant.MEC_ERROR_CONNECTION_TIME_OUT) {
            errorMessage = context.getString(R.string.mec_time_out_error);
        } else if (errorCode == MECConstant.MEC_ERROR_AUTHENTICATION_FAILURE) {
            errorMessage = context.getString(R.string.mec_authentication_failure);
        } else {
            errorMessage = context.getString(R.string.mec_something_went_wrong);
        }
        MECAnalytics.trackAction(MECAnalyticsConstant.SEND_DATA,
                MECAnalyticsConstant.ERROR, MECAnalyticsConstant.SERVER + errorCode + "_" + errorMessage);
        return errorMessage;
    }

    public MECNetworkError createIAPErrorMessage(String pServer, String errorMessage) {
        VolleyError volleyError = new ServerError();
        MECNetworkError error = new MECNetworkError(volleyError, -1, null);
