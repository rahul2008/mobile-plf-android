...
public class ConsentNetworkError implements NetworkErrorListener {
    ...
    private int mErrorCode = ConsentUtil.CONSENT_SUCCESS;
    ...
    private void initErrorCode(final VolleyError error) {
        if (error instanceof NoConnectionError) {
            mErrorCode = ConsentUtil.CONSENT_ERROR_NO_CONNECTION;
        } else if (error instanceof AuthFailureError) {
            mErrorCode = ConsentUtil.CONSENT_ERROR_AUTHENTICATION_FAILURE;
        } else if (error instanceof TimeoutError) {
            mErrorCode = ConsentUtil.CONSENT_ERROR_CONNECTION_TIME_OUT;
        } else if (error instanceof com.android.volley.ServerError) {
            mErrorCode = ConsentUtil.CONSENT_ERROR_SERVER_ERROR;
        } else {
            mErrorCode = ConsentUtil.CONSENT_ERROR_UNKNOWN;
        }
    }
    ...
    private void checkInsufficientStockError(ServerError serverError) {
        ...
        if ("InsufficientStockError".equals(serverError.getErrors().get(0).getType())) {
            mErrorCode = ConsentUtil.CONSENT_ERROR_INSUFFICIENT_STOCK_ERROR;
        }
    }
