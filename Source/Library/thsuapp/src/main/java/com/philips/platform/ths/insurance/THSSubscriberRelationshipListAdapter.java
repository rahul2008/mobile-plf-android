...
public class THSSubscriberRelationshipListAdapter extends BaseAdapter {
    ...
    @Override
    public int getCount() {
        if (null != mTHSRelationship && null != mTHSRelationship.getRelationShipList() && !mTHSRelationship.getRelationShipList().isEmpty()) {
            return mTHSRelationship.getRelationShipList().size();
        } else {
            return 0;
        }
    }
    ...
    public Object getItem(int position) {
        if (null != mTHSRelationship && null != mTHSRelationship.getRelationShipList() && !mTHSRelationship.getRelationShipList().isEmpty()) {
            return mTHSRelationship.getRelationShipList().get(position);
        } else {
            return null;
        }
    }
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        String relationship = mTHSRelationship.getRelationShipList().get(position).getName();
        holder.Relationship.setText(relationship);
        return rowView;
    }
