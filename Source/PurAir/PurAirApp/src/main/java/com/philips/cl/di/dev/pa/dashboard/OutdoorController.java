...
//		OutdoorManager.getInstance().addCurrentCityAreaIDToUsersList(areaId);
	...
	public void onLocationChanged(AMapLocation aLocation) {
		...
		if(location!=null && location.getLatitude() > 0 && location.getLongitude() > 0 
				...
				&& GPSLocation.getInstance().isLocationEnabled()) {
	...
	public void startGetAreaIDTask(double longitude, double latitude) {
		...
		TaskGetHttp citiesList = new TaskGetHttp("http://data.fuwu.weather.com.cn/getareaid/findId?lat=" + latitude + "&lon=" + longitude, "from_lat_long", this);
		citiesList.start();
	}
