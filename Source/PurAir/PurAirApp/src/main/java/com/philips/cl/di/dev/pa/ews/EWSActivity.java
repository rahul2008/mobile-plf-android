...
public class EWSActivity extends ActionBarActivity implements 
	...
	protected void onCreate(Bundle savedInstanceState) {
        ...
        try {
        	/**
			 * When App in background, if we call commit method in place of commitAllowingStateLoss.
	...
	protected void onDestroy() {
		...
		/**
		 * Removed from onStop method,
		 * Due to when App goes to background we need to check network change status.
		 */
	...
	public void showHomeScreen() {
		...
		if (current != null) {
			...
			discoveryManager.insertApplianceToDatabase(current);
	...
	public void onDeviceConnectToHomeNetwork() {
		...
		//Start discovery for all devices
		startDiscovery();
	}
	...
	private void registerWifiNetworkForSocket() {
		...
		/**
		 * Returns the Network currently bound to this process via setProcessDefaultNetwork,
		 *  or null if no Network is explicitly bound.
		 */
		...
		if( setNetwork != null && defaultNetwork == null) {
			ConnectivityManager.setProcessDefaultNetwork(setNetwork);
			Log.e(ALog.WIFI, "EWSActivity$Default Network setProcessDefaultNetwork explicitly");
		}
