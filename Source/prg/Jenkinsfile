...
def MailRecipient = 'DL_CDP2_Callisto@philips.com,DL_URPR@philips.com '
def errors = []
            ...
            if (BranchName =~ /master|develop|release\/platform_.*/) {
                stage ('build') {
                    sh '''#!/bin/bash -l
                        ...
                        ./gradlew -PenvCode=${JENKINS_ENV} assembleRelease cC test jacocoTestReport  saveResDep zipDocuments artifactoryPublish
                    ''' 
                }
            } 
            else {
