...
public class PurifierDatabase implements DICommApplianceDatabase<AirPurifier> {
	...
	public long save(AirPurifier purifier) {
		...
		try {
			...
			values.put(AppConstants.KEY_NOTIFY_PERMISSION, purifier.getNotificationsPermissionEnabled());
	...
	public void loadDataForAppliance(AirPurifier purifier) {
		...
		try {
			...
			if (cursor != null && cursor.getCount() > 0) {
				...
				int notificationPermission = cursor.getInt(cursor.getColumnIndex(AppConstants.KEY_NOTIFY_PERMISSION)) ;
				
				purifier.setLatitude(latitude);
				purifier.setLongitude(longitude);
				purifier.setNotificationsPermissionEnabled(notificationPermission);
			}
			ALog.d(ALog.DATABASE, "Loaded Purifier from db: " + purifier.getNotificationsPermissionEnabled());
		} catch (Exception e) {
