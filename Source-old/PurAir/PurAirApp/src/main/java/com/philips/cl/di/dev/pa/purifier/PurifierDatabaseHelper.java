...
import static com.philips.cdp.dicommclient.networknode.NetworkNodeDatabaseHelper.KEY_ID;
...
public class PurifierDatabaseHelper extends SQLiteOpenHelper {
	...
	public void onCreate(SQLiteDatabase db) {
		...
		String createAirPurifierTableQuery = "CREATE TABLE IF NOT EXISTS " + AppConstants.TABLE_AIRPUR_DEVICE + "(" 
				...
				+ AppConstants.KEY_NOTIFY_PERMISSION + " INTEGER"
				+ ");";
	...
	public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
		...
		if( oldVersion <= 10) {
			...
			try {
				db.execSQL("ALTER TABLE  " + AppConstants.TABLE_AIRPUR_DEVICE + " ADD COLUMN "
						+ AppConstants.KEY_NOTIFY_PERMISSION + " INT");
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
