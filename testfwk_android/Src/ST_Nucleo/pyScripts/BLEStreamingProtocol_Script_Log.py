...
file = open("c:\BleStreamingProtocol.txt", "w")
...
Status = ACI_GAP_SET_DISCOVERABLE(Advertising_Type= 0x00,
...
file.write("-->Rx   :"+ str(dataReceived)+"\n")
...
file.write("<--Tx   :"+ "[0x40,0xFE, 0xFF, 0x07, 0x00, 0x04, 0x00, 0x7B, 0x7D, 0x00]"+"\n")
...
while(True):
 ...
 if(event.get_param('Data_Length').val>2):
  ...
  file.write("<--RxAck:"+ str(dataReceived[0])+"\n") 
  if(dataReceived[0]==0):
   packetId = 63
  else:
  ...
  file.write("<--Tx   :"+ "["+str(packetId)+",0xFE, 0xFF, 0x07, 0x00, 0x04, 0x00, 0x7B, 0x7D, 0x00]"+"\n")  
