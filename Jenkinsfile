    ...
    timestamps {
        try {
            stage('Checkout') {
                ...
                checkout([$class: 'GitSCM', branches: [[name: '*/'+BranchName]], doGenerateSubmoduleConfigurations: false, extensions: [[$class: 'CloneOption', noTags: false, reference: '', shallow: true, timeout: 30],[$class: 'WipeWorkspace'], [$class: 'PruneStaleBranch'], [$class: 'LocalBranch', localBranch: "**"]], recursiveSubmodules: true, submoduleCfg: [], userRemoteConfigs: [[credentialsId: 'd866c69b-16f0-4fce-823a-2a42bbf90a3d', url: 'ssh://tfsemea1.ta.philips.com:22/tfs/TPC_Region24/CDP2/_git/ews-android-easywifisetupuapp']]])
            }
            ...
            if (BranchName =~ /master|develop|release\/platform_.*/) {
                stage('Build') {
                    sh """#!/bin/bash -l
                        ...
                        ./gradlew -PbuildNumber=${env.BUILD_NUMBER} assembleRelease artifactoryPublish
                    """
                }
            } else {
                stage('Build') {
