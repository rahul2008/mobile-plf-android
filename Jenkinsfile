...
def BuildAndUnitTest() {
    sh '''#!/bin/bash -l
        set -e
        chmod -R 755 .
        ./gradlew --refresh-dependencies --full-stacktrace :commlib-integration-tests:testReleaseUnitTest :commlib-ble:testReleaseUnitTest
    '''
...
def PublishUnitTestsResults() {
    ...
    publishHTML([allowMissing: true, alwaysLinkToLastBuild: false, keepAll: true, reportDir: 'Source/cml/Source/Library/commlib-integration-tests/build/reports/tests/testReleaseUnitTest', reportFiles: 'index.html', reportName: 'cml integration tests'])

    def cucumber_path = 'Source/cml/Source/Library/commlib-integration-tests/banana/'

    if (fileExists("$cucumber_path/cucumber.json")) {
