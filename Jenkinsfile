...
node ('Ubuntu && 24.0.3') {
	timestamps {
		stage ('Checkout') {
			checkout([$class: 'GitSCM', branches: [[name: '*/spike']], doGenerateSubmoduleConfigurations: false, extensions: [[$class: 'CleanBeforeCheckout']], submoduleCfg: [], userRemoteConfigs: [[credentialsId: 'acb45cf5-594a-4209-a56b-b0e75ae62849', url: 'ssh://git@atlas.natlab.research.philips.com:7999/mu/mobileuitoolkit-android.git']]])
			step([$class: 'StashNotifier'])
		}
		try {
			stage ('build') {
                sh 'cd ./Source/CatalogApp && chmod -R 775 ./gradlew && ./gradlew clean assembleDebug'
			}
