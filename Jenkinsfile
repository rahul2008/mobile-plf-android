...
node ('android_pipeline &&' + node_ext) {
	timestamps {
		...
		try {
			stage ('build') {
                sh 'chmod -R 775 . && cd ./Source/Library/PrxSample && ./gradlew clean assembleDebug && chmod -R 775 ../../check_and_delete_artifact.sh && ../../check_and_delete_artifact.sh "prx" && ./gradlew assembleRelease zipDocuments artifactoryPublish'
			}
