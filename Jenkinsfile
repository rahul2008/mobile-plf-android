...
node ('android_pipeline &&' + node_ext) {
	timestamps {
        ...
        try {
			stage ('build') {
				sh 'chmod 775 . && cd ./Source/Library && ./gradlew clean assembleDebug && ../../check_and_delete_artifact.sh && ./gradlew lint cC assembleRelease zipDocuments artifactoryPublish'
			}
