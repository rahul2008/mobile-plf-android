...
node ('android_pipeline &&' + node_ext) {
	timestamps {
		...
		try {
			...
			stage ('build') {
                sh 'chmod -R 775 . && cd ./Source/Library/PrxSample && ./gradlew -PenvCode=${JENKINS_ENV} clean assembleDebug && ../../../check_and_delete_artifact.sh "prx" && ./gradlew -PenvCode=${JENKINS_ENV} assembleRelease zipDocuments artifactoryPublish'
				}
			}
			else
			{
			stage ('build') {
				sh 'chmod -R 775 . && cd ./Source/Library/PrxSample && ./gradlew -PenvCode=${JENKINS_ENV} clean assembleDebug assembleRelease'
			}
			}
