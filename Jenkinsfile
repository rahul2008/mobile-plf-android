...
//Cron string to defines a regular interval at which pipeline should be retriggered with parameter buildtype as PSRA and GenerateAPIDocs as true.
...
pipeline {
     ...
     * The values for this user specified parameters are made available to pipeline steps via params.
     */
    parameters {
    ...
    /**
     * trigger parametrizedCron which will re-triggered pipeline at regular interval with parameters available in cron string.
     */
    triggers {
     ...
     */
    options {
        timestamps() //Prepend all console output generated by the pipeline run with the time at which the line was emmited.
    ...
    stages {
        ...
        stage('Commit') {
            //executes current stage if buildType is neither 'PSRA nor 'HPFortify'
            when {
                allOf {
        ...
        stage('Publish to artifactory') {
            //Current stage will be executed only if 1. buildtype is not PSRA and HPFortify 2. branch is master or develop or release/platform_*.
            when {
                allOf {
                    not { expression { return params.buildType == 'PSRA' }}
                    not { expression { return params.buildType == 'HPFortify' }}
