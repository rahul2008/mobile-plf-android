...
class GraphObjectAdapter<T extends GraphObject> extends BaseAdapter implements
		SectionIndexer {
	...
	public void prioritizeViewRange(int firstVisibleItem, int lastVisibleItem,
		...
		// We want to prioritize requests for items which are visible but do not
		// have pictures
		// loaded yet. We also want to pre-fetch pictures for items which are
		// not yet visible
		// but are within a buffer on either side of the visible items, on the
		// assumption that
		// they will be visible soon. For these latter items, we'll store the
		// images in memory
		// in the hopes we can immediately populate their image view when
		// needed.

		// Prioritize the requests in reverse order since each call to
		// prioritizeRequest will just
		// move it to the front of the queue. And we want the earliest ones in
		// the range to be at
		// the front of the queue, so all else being equal, the list will appear
		// to populate from
		// the top down.
		...
		// For items which are not visible, but within the buffer on either
		// side, we want to
		// fetch those items and store them in a small in-memory cache of
		// bitmaps.
		...
		for (int i = start; i < firstVisibleItem; ++i) {
			SectionAndItem<T> sectionAndItem = getSectionAndItem(i);
			if (sectionAndItem.graphObject != null) {
				graphObjectsToPrefetchPicturesFor
						.add(sectionAndItem.graphObject);
			}
		}
		...
		for (T graphObject : graphObjectsToPrefetchPicturesFor) {
			...
			// This URL already have been requested for pre-fetching, but we
			// want to act in an LRU manner, so move
			...
			// If we've already requested it for pre-fetching, no need to do so
			// again.
	...
	protected URI getPictureUriOfGraphObject(T graphObject) {
		...
		if (o instanceof String) {
			...
			ItemPicture itemPicture = GraphObject.Factory
					.create((JSONObject) o).cast(ItemPicture.class);
	...
	protected View getSectionHeaderView(String sectionHeader, View convertView,
			ViewGroup parent) {
		TextView result = (TextView) convertView;

		if (result == null) {
			result = (TextView) inflater.inflate(
					R.layout.com_facebook_picker_list_section_header, null);
		}
	...
	protected View getGraphObjectView(T graphObject, View convertView,
			ViewGroup parent) {
	...
	private View getActivityCircleView(View convertView, ViewGroup parent) {
		...
		if (result == null) {
			result = inflater.inflate(
					R.layout.com_facebook_picker_activity_circle_row, null);
		}
	...
	protected View createGraphObjectView(T graphObject) {
		View result = inflater.inflate(getGraphObjectRowLayoutId(graphObject),
				null);

		ViewStub checkboxStub = (ViewStub) result
				.findViewById(R.id.com_facebook_picker_checkbox_stub);
		...
		ViewStub profilePicStub = (ViewStub) result
				.findViewById(R.id.com_facebook_picker_profile_pic_stub);
		if (!getShowPicture()) {
	...
	protected void populateGraphObjectView(View view, T graphObject) {
		...
		TextView titleView = (TextView) view
				.findViewById(R.id.com_facebook_picker_title);
		if (titleView != null) {
		...
		TextView subtitleView = (TextView) view
				.findViewById(R.id.picker_subtitle);
		...
		if (getShowCheckbox()) {
			CheckBox checkBox = (CheckBox) view
					.findViewById(R.id.com_facebook_picker_checkbox);
		...
		if (getShowPicture()) {
			...
			if (pictureURI != null) {
				ImageView profilePic = (ImageView) view
						.findViewById(R.id.com_facebook_picker_image);
	 ...
	 *             if the GraphObject doesn't have an ID.
	 */
	...
	String getPictureFieldSpecifier() {
		...
		return String.format("picture.height(%d).width(%d)",
				layoutParams.height, layoutParams.width);
	}

	private boolean shouldShowActivityCircleCell() {
		// We show the "more data" activity circle cell if we have a listener to
		// request more data,
		// we are expecting more data, and we have some data already (i.e., not
		// on a fresh query).
		return (cursor != null) && cursor.areMoreObjectsAvailable()
				&& (dataNeededListener != null) && !isEmpty();
	}
	...
	private void rebuildSections() {
		...
		displaySections = sectionKeys.size() > 1
				&& objectsAdded > DISPLAY_SECTIONS_THRESHOLD;
	}
	...
	SectionAndItem<T> getSectionAndItem(int position) {
		...
		if (!displaySections) {
			...
			if (position >= 0 && position < section.size()) {
				graphObject = graphObjectsBySection.get(sectionKey).get(
						position);
			} else {
				// We are off the end; we must be adding an activity circle to
				// indicate more data is coming.
				assert dataNeededListener != null
						&& cursor.areMoreObjectsAvailable();
				// We return null for both to indicate this.
				return new SectionAndItem<T>(null, null);
			}
		} else {
			// Count through the sections; the "0" position in each section is
			// the header. We decrement
			...
			for (String key : sectionKeys) {
				...
				if (position < section.size()) {
					// The position is somewhere in this section. Get the
		...
		if (sectionKey != null) {
			// Note: graphObject will be null if this represents a section
			// header.
	...
	int getPosition(String sectionKey, T graphObject) {
		...
		// First find the section key and increment position one for each header
		// we will render;
		...
		if (!found) {
			...
			// null represents the header for a section; we counted this header
			// in position earlier,
	...
	public int getCount() {
		...
		// If we are not displaying sections, we don't display a header;
		// otherwise, we have one header per item in
		...
		// If we should show a cell with an activity circle indicating more data
		// is coming, add it to the count.
	...
	public Object getItem(int position) {
		SectionAndItem<T> sectionAndItem = getSectionAndItem(position);
		return (sectionAndItem.getType() == SectionAndItem.Type.GRAPH_OBJECT) ? sectionAndItem.graphObject
				: null;
	}

	@Override
	public long getItemId(int position) {
		// We assume IDs that can be converted to longs. If this is not the case
		// for certain types of
		// GraphObjects, subclasses should override this to return, e.g.,
		// position, and override hasStableIds
		// to return false.
		SectionAndItem<T> sectionAndItem = getSectionAndItem(position);
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		switch (sectionAndItem.getType()) {
		case SECTION_HEADER:
			return getSectionHeaderView(sectionAndItem.sectionKey, convertView,
					parent);
		case GRAPH_OBJECT:
			return getGraphObjectView(sectionAndItem.graphObject, convertView,
					parent);
		case ACTIVITY_CIRCLE:
			// If we get a request for this view, it means we need more data.
			assert cursor.areMoreObjectsAvailable()
	...
	public int getSectionForPosition(int position) {
		...
		if (sectionAndItem != null
			...
			return Math.max(0, Math.min(
					sectionKeys.indexOf(sectionAndItem.sectionKey),
					sectionKeys.size() - 1));
		}
		return 0;
	}
	...
	private void downloadProfilePicture(final String profileId, URI pictureURI,
		...
		// If we don't have an imageView, we are pre-fetching this image to
		// store in-memory because we
		// think the user might scroll to its corresponding list row. If we do
		// have an imageView, we
		// only want to queue a download if the view's tag isn't already set to
		// the URL (which would mean
		// it's already got the correct picture).
		boolean prefetching = imageView == null;

		if (prefetching) {
			if (!prefetching) {
				// Setting the tag to the profile ID indicates that we're
				// currently downloading the
				// picture for this profile; we'll set it to the actual picture
				// URL when complete.
			...
			ImageRequest.Builder builder = new ImageRequest.Builder(
					context.getApplicationContext(), pictureURI).setCallerTag(
					this).setCallback(new ImageRequest.Callback() {
				@Override
	...
	private void processImageResponse(ImageResponse response,
			String graphObjectId, ImageView imageView) {
	...
	// Graph object type to navigate the JSON that sometimes comes back instead
	// of a URL string
