...
public class LoginButton extends Button {
	...
	public static enum ToolTipMode {
		/**
		 * Default display mode. A server query will determine if the tool tip
		 * should be displayed and, if so, what the string shown to the user
		 * should be.
		 */
		DEFAULT,

		/**
		 * Display the tool tip with a local string--regardless of what the
		 * server returns
		 */
		DISPLAY_ALWAYS,
	...
	/**
	 * Sets the style (background) of the Tool Tip popup. Currently a blue style
	 * and a black style are supported. Blue is default
	 * 
	 * @param nuxStyle
	 *            The style of the tool tip popup.
	 */
	...
	/**
	 * Sets the mode of the Tool Tip popup. Currently supported modes are
	 * default (normal behavior), always_on (popup remains up until forcibly
	 * dismissed), and always_off (popup doesn't show)
	 * 
	 * @param nuxMode
	 *            The new mode for the tool tip
	 */
	...
	/**
	 * Sets the amount of time (in milliseconds) that the tool tip will be shown
	 * to the user. The default is
	 * {@value ToolTipPopup#DEFAULT_POPUP_DISPLAY_TIME}. Any value that is less
	 * than or equal to zero will cause the tool tip to be displayed
	 * indefinitely.
	 * 
	 * @param displayTime
	 *            The amount of time (in milliseconds) that the tool tip will be
	...
	/**
	 * Gets the current amount of time (in ms) that the tool tip will be
	 * displayed to the user
	 * 
	 * @return
	 */
	...
	public boolean onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (session != null) {
			return session.onActivityResult((Activity) getContext(),
	...
	private void showNuxPerSettings(FetchedAppSettings settings) {
		if (settings != null && settings.getNuxEnabled()
	...
	private void checkNuxSettings() {
		if (nuxMode == ToolTipMode.DISPLAY_ALWAYS) {
			String nuxString = getResources().getString(
			...
			AsyncTask<Void, Void, FetchedAppSettings> task = new AsyncTask<Void, Void, Utility.FetchedAppSettings>() {
				...
				protected FetchedAppSettings doInBackground(Void... params) {
					FetchedAppSettings settings = Utility.queryAppSettings(
							appId, false);
					return settings;
				}
	...
	protected void onVisibilityChanged(View changedView, int visibility) {
		...
		// If the visibility is not VISIBLE, we want to dismiss the nux if it is
		// there
	...
	public void setButtonText(String string) {
		 ...
		 * null) { setText((logoutText != null) ? logoutText :
		 * getResources().getString
		 * (R.string.com_facebook_loginview_log_out_button)); } else {
	...
	/**
	 * Allow a developer to set the OnClickListener for the button. This will be
	 * called back after we do any handling internally for login
	 * 
	 * @param clickListener
	 */
	@Override
