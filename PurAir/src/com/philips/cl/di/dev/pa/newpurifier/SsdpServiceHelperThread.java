...
public class SsdpServiceHelperThread extends HandlerThread {
	...
	public SsdpServiceHelperThread(StartStopInterface startStopInterface, Object startLock, int stopSsdpDelay) {
	...
	protected void onLooperPrepared() {
		mStartStopHandler = new Handler(getLooper()) {
			public void handleMessage(Message msg) {
				...
				switch(msg.what) {
				case MESSAGE_START: mInterface.startDiscoveryFromHandler(); break;
				case MESSAGE_STOP: mInterface.stopDiscoveryFromHandler(); break;
				default: /* NOP */ break;
				}
	...
	public void stopDiscoveryAsync() {
		...
		if (!mStartStopHandler.hasMessages(MESSAGE_STOP)) {
			mStartStopHandler.sendEmptyMessageDelayed(MESSAGE_STOP, mStopSsdpDelay);
	...
	public boolean stopIfNecessary() {
		if (mStartStopHandler.hasMessages(MESSAGE_START)) return false;
