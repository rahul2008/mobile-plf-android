...
public class MapOdActivity extends BaseActivity {
	...
	private void setUpMap() {
		...
		mMap.addMarker(new MarkerOptions().position(new LatLng(centerLatF,
				centerLngF)));
		 ...
		 * mMap.addMarker(new MarkerOptions().position(new LatLng(centerLatF,
		 * centerLngF)).icon(
		 * BitmapDescriptorFactory.fromBitmap(writeTextOnDrawable
		 * (R.drawable.transparent_bg, centerCity))));
		 */

	}

	/*
	 * //Later i will use private Bitmap writeTextOnDrawable(int drawableId,
	 * String text) {
	 * 
	 ...
	 * paint.setColor(Color.BLACK); paint.setTypeface(tf);
	 * paint.setTextAlign(Align.CENTER);
	 * paint.setTextSize(Utils.getPxWithRespectToDip(this, 12));
	 * 
	 * Rect textRect = new Rect(); paint.getTextBounds(text, 0, text.length(),
	 * textRect);
	 * 
	 * Canvas canvas = new Canvas(bm);
	 * 
	 * //Calculate the positions int xPos = canvas.getWidth() / 2 - 2; //-2 is
	 * for regulating the x position offset
	 * 
	 * //"- ((paint.descent() + paint.ascent()) / 2)" is the distance from the
	 * baseline to the center. //int yPos = (int) ((canvas.getHeight() / 2) -
	 * ((paint.descent() + paint.ascent()) / 2)) ; int yPos = canvas.getHeight()
	 * - 5 ; canvas.drawText(text, xPos, yPos, paint);
	 * 
	 * return bm; }
	 */
