...
public class NetworkMonitor {
	...
	public NetworkMonitor(Context context, NetworkChangedCallback listener) {
		...
		mNetworkChangedReceiver = new BroadcastReceiver() {
			...
			public void onReceive(Context context, Intent intent) {
				Log.d("FRANK", "onReceive connectivity action : " + intent.getAction());
	...
	private void updateNetworkState(NetworkState newState) {
		if (mLastKnownState == newState) {
			Log.d("FRANK", "Detected same networkState - no need to update listener");
			return;
		}
		
		Log.d("FRANK", "NetworkState Changed - updating listener");
		...
		public void run() {
			...
			mHandler = new Handler() {
				public void handleMessage(Message msg) {
					...
					if (!isConnected) {
						ALog.d("FRANK", "Network update - No connection");
					...
					if (isMobileData) {
						...
						ALog.d("FRANK", "Network update - Mobile connection");
					...
					if (isInternetAvailable) {
						ALog.d("FRANK", "Network update - Wifi with internet");
						updateNetworkState(NetworkState.WIFI_WITH_INTERNET);
						return;
					}
					
					ALog.d("FRANK", "Network update - Wifi without internet");
	...
	private boolean checkForInternetConnection() {
		...
		try {
			...
			if (response.getStatusLine().getStatusCode() == HttpStatus.SC_OK) {
				ALog.d("FRANK", "Internet connection available");
				return true;
			}
