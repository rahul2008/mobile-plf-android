...
public class WeatherReportLayout extends  LinearLayout {
	...
	public WeatherReportLayout(final Context context, AttributeSet attrs, String timeStr) {
		...
		weatherDetails = HomeFragment.getWeatherDetails();
		...
		calender = Calendar.getInstance(TimeZone.getTimeZone("GMT+8"));
		
		hourOfDay = calender.get(Calendar.HOUR_OF_DAY);
		
		dayoffWeek = calender.get(Calendar.DAY_OF_WEEK);
		
	}
	
	public void addTodayWeatherInfo() {
	...
	public void ShowTodayWeather(int num) {
		for (int i = 0; i < num; i++) {
	...
	public void addFiveDayWeatherInfo() {
		...
		for (int j = 0; j < nextFiveDays.length; j++) {
			...
			String dayStr = Utils.getDayOfWeek(context, dayoffWeek);
	...
	public void ShowNextFiveDaysWeather(int num) {
		int count = 9;
		for (int i = 0; i < num; i++) {
